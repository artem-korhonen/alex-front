...
function StartButtons({ id, isLiked, songs, like }: StartButtonsProps) {
    return (
        <div className="flex flex-col md:flex-row gap-5 text-[var(--color-text)] w-full">
            <div className="bg-[var(--color-bg-black)] flex flex-1 items-center justify-center gap-5 font-bold border border-[var(--color-primary)] cursor-pointer transition-transform transition-colors hover:scale-102 hover:bg-[var(--color-primary-darker)] rounded rounded-2xl p-5" onClick={() => {like(id, !isLiked)}}>
                <Heart className={`${isLiked ? "fill-[var(--color-text)]" : ""}`} />
                <h2>Нравится</h2>
            </div>
